...
function createtime1() {
  var grt = new Date("12/10/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by SIRIUSPLUTO %c 你正在访问SIRIUSPLUTOの小破站",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| SIRIUSPLUTO") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('SIRIUSPLUTO的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("12/10/2023 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("12/10/2023 00:00:00");
